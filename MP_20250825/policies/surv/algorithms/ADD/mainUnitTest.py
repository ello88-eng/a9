...
from model.surv.darp.multi_robot_path_planner import MultiRobotPathPlanner
...
@parameterized_class(
    ("multiRobot", "case"), [(cases[0], 1), (cases[1], 2), (cases[2], 3), (cases[3], 4)]
)
class TestDARP(unittest.TestCase):
    def test_DARP_assignment_matrix(self):
        ...
        assert_true(
            np.allclose(
                self.multiRobot.darp_instance.A, groundTruthAssignmentMatrix, atol=1
            )
        )
