...
from config.mp_config import DARP_THRESHOLD
...
class Darp:
    def __init__(
        ...
        for r in range(self.num_uavs):
            # np.random.seed(r)
            self.colors.append(np.random.choice(range(256), size=3).tolist())
    ...
    def divide_regions(self):
        ...
        while self.termThr <= self.dcells and not success and not cancelled:
            ...
            while iteration <= self.MaxIter and not cancelled:
                ...
                if self.IsThisAGoalState(DARP_THRESHOLD, ConnectedRobotRegions):
                    break
    ...
    def construct_Assignment_Matrix(self):
        ...
        for i in range(self.num_uavs):
            ...
            if DesireableAssign[i] != true_round(DesireableAssign[i]) and termThr != 1:
                termThr = 1
