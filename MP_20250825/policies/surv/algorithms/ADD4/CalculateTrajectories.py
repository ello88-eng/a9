...
from policies.surv.algorithms.ADD3.Edges import Edge
...
class CalculateTrajectories:
    ...
    def CalculatePathsSequence(self, StartingNode):
        ...
        while True:
            ...
            found = False
            for idx in range(4):
                if (prevNode + movement[(idx + offset) % 4] in self.nodes[prevNode]) and not (
                    prevNode + movement[(idx + offset) % 4] in RemovedNodes
                ):
            ...
            i = true_round(currentNode / (2 * self.cols))
            j = currentNode % (2 * self.cols)
            previ = true_round(prevNode / (2 * self.cols))
