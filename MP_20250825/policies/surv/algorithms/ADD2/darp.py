...
from policies.surv.algorithms.ADD2.Visualization import darp_area_visualization
...
@njit(fastmath=True)
def euclidian_distance_points2d(array1: np.array, array2: np.array) -> float:
...
class DARP:
    ...
    def __init__(
        ...
        self.rows = nx
        self.cols = ny
        self.initial_positions, self.obstacles_positions, self.portions = self.sanity_check(
            given_initial_positions,
            given_portions,
            obstacles_positions,
            notEqualPortions,
    ...
    def generateRandomMatrix(self):
        ...
        RandomMatrix = 2 * self.randomLevel * rng.uniform(0, 1, size=(self.rows, self.cols)) + (1 - self.randomLevel)
        return RandomMatrix
