...
class visualize_paths:
    def __init__(self, AllRealPaths, subCellsAssignment, DroneNo, color):
        ...
        self.dimensions = min_max_scaler.fit_transform(
            np.array([self.subCellsAssignment.shape[0], self.subCellsAssignment.shape[1], 0]).reshape(-1, 1)
        ).ravel()
    ...
    def visualize_paths(self, mode):
        ...
        while True:
            ...
            self.drawSquareGrid(
                self._VARS["gridOrigin"], self._VARS["gridWH"], self._VARS["gridCellsX"], self._VARS["gridCellsY"]
            )
            self.placeCells()
            ...
            for point in self.AllRealPaths[r]:
                ...
                pygame.draw.line(
                    ...
                    (
                        ...
                        self._VARS["gridOrigin"][1] + (celldimY * point[2]) + celldimY / 2,
                    ),
                    width=4,
                )
        ...
        for row in range(self.subCellsAssignment.shape[0]):
            for column in range(self.subCellsAssignment.shape[1]):
                if self.subCellsAssignment[row][column] == self.DroneNo:
                    self.drawSquareCell(
                        self._VARS["gridOrigin"][0] + (celldimX * column) + self._VARS["lineWidth"] / 2,
                        self._VARS["gridOrigin"][1] + (celldimY * row) + self._VARS["lineWidth"] / 2,
                        celldimX,
                        celldimY,
                        BLACK,
                    )
...
class darp_area_visualization(object):
    def __init__(self, Assignment_matrix, DroneNo, color, init_robot_pos):
        ...
        dimensions = min_max_scaler.fit_transform(
            np.array([self.Assignment_matrix.shape[0], self.Assignment_matrix.shape[1], 0]).reshape(-1, 1)
        ).ravel()
