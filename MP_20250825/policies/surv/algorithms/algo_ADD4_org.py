...
from config.mp_config import ALT_REF, FOV_LEN, SURV_TURN_RADIUS
...
def select(manager: Manager) -> tuple[float, Dict[int, SurveillanceSelectionResult], MissionPlanInput]:
    ...
    modi_xs = np.linspace(top_left[1], top_right[1], int(mesh_xs.shape[0] / 4))
    modi_ys = np.linspace(bottom_right[0], top_right[0], int(mesh_ys.shape[0] / 4))
    ...
    obs_pos, _, _ = pgp.get_obstacle_position_from_polygon_latlon(
        [top_left, bottom_left, top_right, bottom_right], manager.mp_input.polygon_area.vertices, use_hull=False
    )
