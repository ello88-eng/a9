...
from policies.surv.algorithms.ADD3.darp import Darp
from policies.surv.algorithms.ADD3.kruskal import Kruskal
from policies.surv.algorithms.ADD3.turns import turns
...
def get_obstacle_positions(manager: Manager) -> List[int]:
    """임무영역 내 임무가능영역 및 기 촬영 영역이 아닌 곳은 obstacle로 처리하는 함수.\n
    `Obstacle position := Boundary area - (Polygon area + Circular area + Grid coverage area)`

    Args:
        manager (Manager): 임무표적관리 클래스 인스턴스.

    Returns:
        List[int]: 임무영역 내 임무가능영역 및 기 촬영 영역이 아닌 곳을 표현하는 1D 리스트.
    """
    pass


class MultiRobotPathPlanner_ADD3:
    def __init__(
        self,
        nx,
        ny,
        ...
        if not self.DARP_success:
            ...
            for value in range(self.darp_instance.num_uavs):
                ...
                line_count = int(max_grid / s_beta + 10)
                line_spacing = s_beta
                lines = []
...
def select(manager: Manager) -> tuple[float, Dict[int, SurveillanceSelectionResult], MissionPlanInput]:
    ...
    org_xs = np.linspace(top_left[0], top_right[0], mesh_xs.shape[0])
    org_ys = np.linspace(bottom_right[1], top_right[1], mesh_ys.shape[0])
    modi_xs = np.linspace(top_left[0], top_right[0], int(mesh_xs.shape[0] / 4))
    modi_ys = np.linspace(bottom_right[1], top_right[1], int(mesh_ys.shape[0] / 4))
        ...
        if idx in in_pos:
            ...
            while True:
                ...
                if (idx not in in_pos) and (idx < mesh_x.shape[0] * mesh_y.shape[0]):
                    break
        in_pos.append(int(y_idx * mesh_x.shape[0] + x_idx))

    # 임무영역 내 임무가능영역이 아닌 곳은 obstacle 처리
    obs_pos = get_obstacle_positions(manager)
    ...
    turn_radius = 300
    fov_len = 200  # 기존: 50
    mrpp = MultiRobotPathPlanner_ADD3(
        nx=grid[0],
        ny=grid[1],
        notEqualPortions=nep,
        initial_positions=in_pos,
        portions=portions,
        obs_pos=obs_pos,
        visualization=vis,
        map_roi=map_roi,
    ...
    for i in range(len(avs_pos_list)):
        ...
        for j in range(len(grid_path)):
            ...
            yy.append(tmp_y)
        waypoint_array = get_coordinate_array(xx, yy, ALT_REF)
        ...
        waypoint_array_as_lla[:, -1] = manager.mp_input.mission_init_info.surv_alt
        ...
        mp_result_dict.update(
            {
                avs_info.avs_id: SurveillanceSelectionResult(
                    ...
                    waypoints=waypoint_array_as_lla,
                )
            }
        )
