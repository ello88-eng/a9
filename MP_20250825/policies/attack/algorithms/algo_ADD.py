...
class ADDAttackAlgorithmRunner:
    ...
    def select(self) -> Tuple[float, Dict[int, SurveillanceSelectionResult], MissionPlanInput]:
        ...
        Returns:
            Tuple[float, Dict[int, MissionPlanOutput], MissionPlanInput]: _description_
        """
        # 타격 알고리즘 결과 초기화
        ...
        for trg_id, fus_out in self.manager.accum_trg_fus_res.items():
            ...
            if self.manager.trg_to_state_dict[trg_id] == 1:
                ...
                if self.manager.use_prev_mp_output:
                    ...
                    if not hasattr(self.manager, "prev_mp_output_dict"):
                        return 0, mp_output_dict, MissionPlanInput()
                ...
                # 이전 임무계획 결과를 사용하지 않는 경우
                    ...
                    for avs_id, info in self.manager.mp_input.avs_info_dict.items():
                        if info.avs_system_mode == 2 and info.system_group_id == trg_id:
                            rd_to_attack[trg_id]["total_avs"].append(avs_id)
                            dist = np.linalg.norm(
...
class ADDPartialAttackAlgorithmRunner:
    def __init__(self, manager: Manager) -> None:
        self.manager = manager
